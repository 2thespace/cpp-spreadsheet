...
void TestFormulaReferences() {
    ...
    auto var = ParseFormula(std::move("A1"))->Evaluate(*sheet);
...
void TestErrorValue() {
    ...
    auto val = sheet->GetCell("E4"_pos)->GetValue();
...
void TestCellReferences() {
    ...
    auto v = sheet->GetCell("A2"_pos)->GetReferencedCells();
...
int main() {
    ...
    RUN_TEST(tr, TestCellCircularReferences); // FAIL
}
